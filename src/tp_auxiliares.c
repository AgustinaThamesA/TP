...
int calculo_puntaje(TP *tp)
{
	...
	puntaje = 100 - (100 * (abs(tiempo_jugador1 - tiempo_jugador2) /
				(tiempo_jugador1 - tiempo_jugador2)));
...
void establecer_dificultad(TP *tp, int dificultad)
{
	...
	pista_jugador_t *pista_jugador_1 =
		tp->jugadores.pista_jugador[JUGADOR_1];
	pista_jugador_t *pista_jugador_2 =
		tp->jugadores.pista_jugador[JUGADOR_2];
...
void seleccionar_dificultad(TP *tp)
{
	...
	if (scanf("%d", &dificultad) != 1 && scanf("%d", &dificultad) != 2 &&
	    scanf("%d", &dificultad) != 3 && scanf("%d", &dificultad) != 4) {
...
void seleccionar_pokemon(TP *tp)
{
	...
	const struct pokemon_info *poke_usuario =
		tp_pokemon_seleccionado(tp, JUGADOR_1);
...
void mostrar_menu_principal(int *opcion)
{
	...
	if (scanf("%d", opcion) != 1 && scanf("%d", opcion) != 2 &&
	    scanf("%d", opcion) != 3) {
		fprintf(stderr, "Error al leer la opción\n");
	}
	printf("\n\n\n");
...
{
	for (int i = 0;
	     i < tp->jugadores.pista_jugador[JUGADOR_2]->max_obstaculos; i++) {
		...
		unsigned obstaculo_posicion = aleatoria(
			(int)tp->jugadores.pista_jugador[JUGADOR_2]->largo_pista,
			0);
		tp_agregar_obstaculo(tp, JUGADOR_2, obstaculo_aleatorio,
				     obstaculo_posicion);
	}
}
...
void usuario_elije_pista(TP *tp)
{
	...
	for (int i = 0;
		...
		tp_agregar_obstaculo(tp, JUGADOR_1, obstaculo_aleatorio,
				     (unsigned)posicion);
...
{
	const struct pokemon_info *pokemon_jugador =
...
void correr_carrera(TP *tp)
{
	...
	double velocidad_jugador1 = (pista_jugador_1->largo_pista) /
				    tp_calcular_tiempo_pista(tp, JUGADOR_1);
	double velocidad_jugador2 = (pista_jugador_2->largo_pista) /
	...
	while (distancia_recorrida_jugador1 <= pista_jugador_1->largo_pista &&
		...
		if (turno % 2 == 0) {
			velocidad_jugador1 = calcular_velocidad(
				tp, JUGADOR_1,
				(size_t)distancia_recorrida_jugador1);
		} else {
			velocidad_jugador2 = calcular_velocidad(
				tp, JUGADOR_2,
				(size_t)distancia_recorrida_jugador2);
		}
